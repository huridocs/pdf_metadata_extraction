...
class TestTasks(TestCase):
    ...
    def test_add_prediction_task(self):
        tenant = 'tenant_one'
        template = 'template_one'
        pdf_file_name = 'test.pdf'
        mongo_client = pymongo.MongoClient('mongodb://mongo_paragraphs:27017')

        tasks = Tasks(tenant, template)
        with open(f'{ROOT_FOLDER}/test_files/test.pdf', 'rb') as file:
            tasks.add(pdf_file_name, file.read())

        document = mongo_client.pdf_paragraph.tasks.find_one(
            {'tenant': tenant, 'template': template, 'pdf_file_name': pdf_file_name})
        task = CreateModelTask(**document)

        self.assertTrue(os.path.exists(f'{DOCKER_VOLUME_PATH}/to_extract/{tenant}/{template}/{pdf_file_name}'))
    ...
    def test_get_next_task(self):
        ...
        for i in range(10):
            task = CreateModelTask(tenant=tenant + str(i), pdf_file_name=pdf_file_name + str(i))
