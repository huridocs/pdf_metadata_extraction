...
class TestXmlFile(TestCase):
    ...
    def test_save_xml(self):
        ...
        with open(self.test_file_path, 'rb') as file:
            xml_file = XmlFile(tenant=tenant, template=template, property_name=property_name, file_name='test.xml')
            xml_file.save_as_to_train(file=file.read())
            xml_file.save_as_to_predict(file=file.read())

        base_path = f'{DOCKER_VOLUME_PATH}/{tenant}/{template}/{property_name}'
        self.assertTrue(os.path.exists(f'{base_path}/to_train_xml/test.xml'))
        self.assertTrue(os.path.exists(f'{base_path}/to_predict_xml/test.xml'))
