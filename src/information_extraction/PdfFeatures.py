...
class PdfFeatures:
    def __init__(self, segment_tags: List[SegmentTag]):
        ...
        self.fonts = list(
            {segment_tag.font for segment_tag in segment_tags if segment_tag.font}
        )
    ...
    def get_modes(self):
        ...
        for segment_tag in self.segment_tags:
            ...
            bottom_tags_tops = [
                ...
                and bottom_tag.top > segment_tag.bottom
            ]
        ...
        self.font_family_mode = abs(
            int(
                str(hashlib.sha256(font_family_name_mode.encode("utf-8")).hexdigest())[
                    :8
                ],
                16,
            )
        )
        self.font_family_mode_normalized = float(
            f"{str(self.font_family_mode)[0]}.{str(self.font_family_mode)[1:]}"
        )
