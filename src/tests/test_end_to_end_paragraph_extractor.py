...
Ipsum."""
    ...
    def test_extract_paragraphs_via_endpoints(self):
        response = requests.post(f"{SERVER_URL}/extract_paragraphs", files=[])
        self.assertEqual(422, response.status_code)
        ...
        paragraphs_translations = self.wait_for_paragraphs_translations("key_2")
        self.assertIsNotNone(paragraphs_translations)
        self.assertEqual("key_2", paragraphs_translations.key)
        self.assertEqual("en", paragraphs_translations.main_language)
        self.assertEqual(["en", "fr"], paragraphs_translations.available_languages)
...
Ipsum."""
    ...
    @staticmethod
    def wait_for_paragraphs_translations(key: str) -> ParagraphsTranslations | None:
        url = f"{SERVER_URL}/get_paragraphs_translations/{key}"
        for _ in range(20):
            time.sleep(3)
            response = requests.get(url)
            if response.status_code == 200 and response.json().get("paragraphs"):
                return ParagraphsTranslations(**response.json())
        return None
