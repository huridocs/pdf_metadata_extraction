...
class T5MethodEarlyStopping(Method):
    ...
    def train(self, semantic_extraction_data: List[SemanticExtractionData]):
        ...
        model_args.cache_dir = f"{self.base_path}/cache_t5_early"

        model = T5Model("t5", "t5-small", args=model_args, use_cuda=torch.cuda.is_available())

        model.train_model(train_df, eval_data=train_df)
    ...
    def predict(self, texts: List[str]) -> List[str]:
        ...
        predictions = model.predict([f"{self.property_name}: {input_text}" for input_text in texts])
        return predictions
